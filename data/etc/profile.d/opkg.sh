opkg() {
	...
	case "${OPKG_CMD}" in
		(init|uci|import|save|restore|rollback|upgr|export|newconf|proc|reinstall) opkg_"${@}" ;;
		(*) command opkg "${@}" ;;
	esac
}
...
opkg_uci() {
	...
	if ! uci -q get opkg > /dev/null; then opkg init; fi
	uci -q batch << EOI
...
opkg_import() {
	local OPKG_OPT="${1:-${OPKG_UCI}}"
	if [ -e "${OPKG_OPT}" ]; then sed -n -r -e "s/\s(overlay|unknown)$/tipkg/p" "${OPKG_OPT}" | opkg uci auto; fi
}
...
opkg_save() {
	...
	if uci -q get fstab.rwm > /dev/null && grep -q -e "\s/rwm\s" /etc/mtab; then {
...
opkg_restore() {
	...
	grep -x -f "${OPKG_AI}" "${OPKG_PR}" | opkg proc remove
	grep -v -x -f "${OPKG_AI}" "${OPKG_PI}" | opkg proc install
...
opkg_rollback() {
	...
	local OPKG_PR="$(opkg export pr)"
	local OPKG_PI="$(opkg export pi)"
	if uci -q get opkg."${OPKG_CONF}" > /dev/null; then
		opkg restore "${OPKG_CONF}"
		grep -v -x -f "${OPKG_PI}" "${OPKG_UI}" | opkg proc remove
		grep -v -x -f "${OPKG_PR}" "${OPKG_UR}" | opkg proc install
	fi
...
opkg_export_type() {
	...
	find "${OPKG_INFO}" -name "*.control" -type "${OPKG_TYPE}" 2> /dev/null | sed -e "s/^.*\///;s/\.control$//"
}
...
opkg_export_uci() {
	...
	uci -q get opkg."${OPKG_CONF}"."${OPKG_TYPE}" | sed -e "s/\s/\n/g"
}
